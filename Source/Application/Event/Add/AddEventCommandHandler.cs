...
namespace Application.Event.Add
{
    public class AddEventCommandHandler : IBaseCommandHandler<AddEventCommand>
    {
        ...
        {
            var Event = new Domain.EventAgg.Event(request.userNumber, request.Title, DateTime.Parse(request.StartTime), DateTime.Parse(request.EndTime),request.Description, request.Link, request.EventAddress, request.tag,request.notification,request.accessNotification);
